...
package com.delozoya.nuclearota.xml;
...
public class OTAParser {
    ...
    private static final String FILENAME_TAG_PATH = "Path";
    ...
    private void readDevice(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            }else if(tagName.equalsIgnoreCase(FILENAME_TAG_PATH)) {
                String tagValue = readTag(parser, tagName);
                mDevice.setFileLink(tagValue);
                Log.d("Test",tagValue);
            }else{
                skip(parser);
            }
